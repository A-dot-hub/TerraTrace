...
import { generateDemoActivities, DEMO_GOALS, DEMO_SCENARIOS, DEMO_NEWS } from '../utils/demoData';
import { calculateEmission, calculateSustainabilityScore, parseQuickTrace, EMISSION_FACTORS } from '../utils/carbonCalculator';
...
export function computeDashboardAnalytics() {
  ...
  // Current 7 days
  const thisWeekActs = activities.filter((a) => new Date(a.timestamp) >= sevenDaysAgo);
  // Previous 7 days
  ...
  const weeklyFootprint = Number(thisWeekActs.reduce((acc, a) => acc + (a.emission || 0), 0).toFixed(1));
  const prevWeekFootprint = Number(lastWeekActs.reduce((acc, a) => acc + (a.emission || 0), 0).toFixed(1)) || 58.2;

  const weeklyChangePercent = prevWeekFootprint > 0 
    ? Number((((weeklyFootprint - prevWeekFootprint) / prevWeekFootprint) * 100).toFixed(1))
    : 0;
  ...
  const totalCatEmissions = Object.values(categoryTotals).reduce((a, b) => a + b, 0) || 1;
  ...
  const biggestImpact = categoryBreakdown[0] || { name: 'Transportation', percentage: 51, value: 24.5 };
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const dayActs = activities.filter((a) => a.timestamp && a.timestamp.startsWith(dayStr));
  ...
  let recommendation = {
    ...
    description: 'Replacing 36 km of private driving with rapid transit cuts approximately 5.8 kg CO2e weekly.',
  ...
  if (biggestImpact.name === 'Food') {
    recommendation = {
      ...
      description: 'Replacing high-emission meals with legumes or vegetarian bowls cuts ~4.2 kg CO2e weekly.',
    ...
    recommendation = {
      ...
      description: 'Auditing standby AC and continuous appliance draw can trim up to 6 kWh per week.',
    ...
    recommendation = {
      ...
      description: 'Diverting 1.5 kg of organic waste prevents anaerobic landfill methane formation.',
  ...
  const projection = [
    { period: 'Current', baseline: Number(weeklyFootprint.toFixed(1)), optimized: Number(weeklyFootprint.toFixed(1)) },
    { period: '1 Month', baseline: Number((weeklyFootprint * 4.3).toFixed(1)), optimized: Number((weeklyFootprint * 4.3 * 0.82).toFixed(1)) },
    { period: '6 Months', baseline: Number((weeklyFootprint * 26).toFixed(1)), optimized: Number((weeklyFootprint * 26 * 0.76).toFixed(1)) },
    { period: '1 Year', baseline: Number((weeklyFootprint * 52).toFixed(1)), optimized: Number((weeklyFootprint * 52 * 0.72).toFixed(1)) },
  ];
...
export const api = {
  ...
  async addActivity(data) {
    try {
      ...
      const activities = getLocal(STORAGE_KEYS.ACTIVITIES, []);
      const emission = data.emission !== undefined 
        ? data.emission 
  ...
  async simulateFuture(inputs) {
    try {
      ...
      const newMetro = inputs.metroKm !== undefined ? Number(inputs.metroKm) : curMetro;
      const newChicken = inputs.chickenMeals !== undefined ? Number(inputs.chickenMeals) : curChicken;
      const newElec = inputs.electricityKwh !== undefined ? Number(inputs.electricityKwh) : curElec;
      const newWaste = inputs.foodWasteKg !== undefined ? Number(inputs.foodWasteKg) : curWaste;
      ...
      const weeklyDiff = Number((currentFootprint - simulatedFootprint).toFixed(1));
      const percentageReduction = currentFootprint > 0
  ...
  async saveScenario(scenario) {
    try {
      const res = await apiClient.post('/api/scenarios', scenario);
      return res.data;
    } catch {
  ...
  async deleteScenario(id) {
    try {
      ...
      const list = getLocal(STORAGE_KEYS.SCENARIOS, []);
      setLocal(STORAGE_KEYS.SCENARIOS, list.filter((s) => s.id !== id));
    }
    return { success: true };
  },
  ...
  async deleteGoal(id) {
    try {
      ...
      const list = getLocal(STORAGE_KEYS.GOALS, []);
      setLocal(STORAGE_KEYS.GOALS, list.filter((g) => g.id !== id));
    }
    return { success: true };
  },
  ...
  async askTraceAI(question) {
    try {
      ...
      if (q.includes('why') && (q.includes('high') || q.includes('footprint') || q.includes('score'))) {
        return {
      ...
      if (q.includes('biggest') || q.includes('largest') || q.includes('source') || q.includes('opportunity')) {
        return {
      ...
      if (q.includes('change first') || q.includes('start') || q.includes('priority')) {
        return {
